...
class Splunkbase(object):
    ...
    def login(self, username, password):
        post_args = urlencode(dict(username=username, password=password))
        # Forward post arguments, including username and password.
        with closing(urlopen(self.login_url, post_args, URLOPEN_TIMEOUT)) as f:
    ...
    def download_appfile(self, fd, appid, version):
        ...
        if location:
            req = Request(url=location)
            req.add_header('X-Auth-Token', unquote(self.sessionKey))
            req.add_header('Content-Type', 'application/x-gzip')
            
            to_file = open(fd, 'w+')
            with nested(closing(urlopen(req)), to_file) as (remote, local):
