define(["api/SplunkVisualizationBase","api/SplunkVisualizationUtils","jquery"], function(__WEBPACK_EXTERNAL_MODULE_1__, __WEBPACK_EXTERNAL_MODULE_2__, __WEBPACK_EXTERNAL_MODULE_4__) { return /******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _SplunkVisualizationBase = __webpack_require__(1);

	var _SplunkVisualizationBase2 = _interopRequireDefault(_SplunkVisualizationBase);

	var _SplunkVisualizationUtils = __webpack_require__(2);
	...
	var SimpleTimeseries = function (_SplunkVisualizationB) {
	    _inherits(SimpleTimeseries, _SplunkVisualizationB);

	    function SimpleTimeseries() {
	        _classCallCheck(this, SimpleTimeseries);

	        return _possibleConstructorReturn(this, (SimpleTimeseries.__proto__ || Object.getPrototypeOf(SimpleTimeseries)).apply(this, arguments));
	    }

	    _createClass(SimpleTimeseries, [{
	        key: 'initialize',
	        value: function initialize() {
	            _SplunkVisualizationBase2.default.prototype.initialize.apply(this, arguments);
	        ...
	        key: '_getEscapedProperty',
	        value: function _getEscapedProperty(name, config) {
	            var propertyValue = config[this.getPropertyNamespaceInfo().propertyNamespace + name];
	            return _SplunkVisualizationUtils2.default.escapeHtml(propertyValue);
	        }
	    }, {
	        key: 'formatData',
	        value: function formatData(data, config) {
	            if (!data) {
	                return false;
	            }
	            var formattedData = {
	            ...
	            var seriesNames = data.rows.map(function (row) {
	                return row[1];
	            ...
	            data.rows.map(function (row) {
	                ...
	                var val = window.isNaN(+row[2]) ? 0 : +row[2];
	                var color = row[3] || false;
	            ...
	            // uniquify
	            statistics.times = times.filter(function (value, index, array) {
	                return array.indexOf(value) === index;
	            });
	            statistics.min = min;
	            statistics.max = max;
	            var orderedGroupedBySeries = {};

	            Object.keys(groupedBySeries).forEach(function (key) {
	                var series = groupedBySeries[key];
	        ...
	        value: function updateView(data, config) {
	            ...
	            this.svgEl.attr('width', this.$el.width()).attr('height', this.$el.height());
	            this.gEl = this.svgEl.append('g').attr("transform", "translate(" + margin.left + "," + margin.top + ")");
	            ...
	            yScale.domain([data.statistics.min, data.statistics.max]);

	            xScale.domain(d3.extent(data.statistics.times));

	            this.gEl.append('g').attr('class', 'axis x').attr('transform', 'translate(0, ' + height + ')').call(d3.axisBottom(xScale));

	            this.gEl.append("g").attr("class", "axis y").attr('transform', 'translate(0,0)').call(d3.axisLeft(yScale)).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", "0.71em").attr("fill", "#000").text(data.fields[2] ? data.fields[2]['name'] : '');

	            var series = Object.values(data.series);
	            var seriesNames = Object.keys(data.series);

	            var serie = this.gEl.selectAll('.serie').data(series).enter().append('g').attr('class', 'serie');

	            var $tooltip = (0, _jquery2.default)('<div class="ttt"></div>');
	            $tooltip.appendTo(this.$el);

	            serie.append('path').attr('class', 'line').attr('d', function (d) {
	                return line(d);
	            }).style('fill', 'none').style("stroke", function (d, i) {
	                var color = d[0]['color'];return color ? color : colorScale(i);
	            }).style('stroke-width', '1.5px');

	            serie.append('g').selectAll('circle').data(function (d) {
	                return d;
	                ...
	                return d3.select(this.parentNode.parentNode).select('.line').style('stroke');
	            }).style('stroke-width', '1.5px').attr('r', function () {
	                return showDatapoints ? 2 : 0;
	            ...
	            var LEGEND_RECT_SIZE = 16;
	            var LEGEND_SPACING = 3;
	            var LEGEND_WIDTH = margin.right - 20;
	            var LEGEND_MARGIN_TOP = margin.top;

	            if (showLegend) {

	                var legend = this.svgEl.selectAll('.legend').data(colorScale.domain()).enter().append('g').attr('class', 'legend').attr('transform', function (d, i) {
	                    ...
	                    serie.selectAll('.line').transition(200).style('opacity', function () {
	                ...
	                legend.append('rect').attr('width', LEGEND_RECT_SIZE).attr('height', LEGEND_RECT_SIZE).attr('fill', colorScale).attr('fill-opacity', .5).attr('stroke-width', 1).style('stroke', colorScale);

	                legend.append('text').attr('x', LEGEND_RECT_SIZE + 2 * LEGEND_SPACING).attr('y', LEGEND_RECT_SIZE - LEGEND_SPACING + LEGEND_RECT_SIZE / 6).text(function (d, i) {
	                    return truncate(seriesNames[i]);
	        ...
	        key: 'getInitialDataParams',
	        value: function getInitialDataParams() {
	            return {
	...
	}(_SplunkVisualizationBase2.default);
