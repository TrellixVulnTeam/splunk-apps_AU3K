    ...
    <endpoint>
        <args>    
            ...
            <arg name="ca_bundle_path">
                ...
                <description>Full path to your CA Bundle , https://requests.readthedocs.io/en/master/user/advanced/#ssl-cert-verification</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
            </arg>
            <arg name="client_cert_path">
                <title>Client Certificate</title>
                <description>Full path to your client certificate</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
            </arg>
            <arg name="client_key_path">
                <title>Client Unencrypted Private Key</title>
                <description>Full path to your unencrypted private key</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
            </arg>
            <arg name="client_bundled_path">
                <title>Bundled Client Certificate/Unencrypted Private Key</title>
                <description>Alternatively to declaring your certificate and key seperately above , you can enter the full path to your bundled client certificate/unencrypted private key file</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
            </arg>
...
def do_run(config,endpoint_list):
    ...
    if len(activation_key) > 32:
        ...
        if not m.hexdigest().upper() == activation_hash.upper():
            logging.error("%s : FATAL Trial Activation key for App '%s' failed. Please ensure that you copy/pasted the key correctly." % (STANZA,app_name))
            sys.exit(2)
        if ((current_ts - int(activation_ts)) > 604800):
            logging.error("%s : FATAL Trial Activation key for App '%s' has now expired. Please visit http://www.baboonbones.com/#activation to purchase a non expiring key." % (STANZA,app_name))
            sys.exit(2)
    else:
        m = hashlib.md5()
        m.update((app_name).encode('utf-8'))
        if not m.hexdigest().upper() == activation_key.upper():
            logging.error("%s : FATAL Activation key for App '%s' failed. Please ensure that you copy/pasted the key correctly." % (STANZA,app_name))
            sys.exit(2)
    ...
    try:
        ...
        logging.error("%s : Looks like an error reading in stateful params %s" % (STANZA,str(e)))
    ...
    try: 
        ...
        while True:
            ...
            for endpoint in endpoint_list:
                ...
                try:
                    ...
                    logging.error("%s : HTTP Request Timeout error: %s" % (STANZA,str(e)))
                    time.sleep(float(backoff_time))
                    continue
                except Exception as e:
                    logging.error("%s : Exception performing request: %s" % (STANZA,str(e)))
                    time.sleep(float(backoff_time))
                    continue
                try:
                    r.raise_for_status()
                    ...
                    if index_error_response_codes:
                        ...
                        sys.stdout.flush()
                    logging.error("%s : HTTP Request error: %s" % (STANZA,str(e)))
                    time.sleep(float(backoff_time))
                    continue
        ...
        logging.error("%s : Looks like an error: %s" % (STANZA,str(e)))
        sys.exit(2) 
...
def replaceTokens(raw_string):
    ...
    try:
        ...
        e = sys.exc_info()[1]
        logging.error("%s : Looks like an error substituting tokens: %s" % (STANZA,str(e))) 
...
def checkParamUpdated(cached,current,rest_name,credentials_list):
    ...
    if not (cached == current):
        try:
            ...
            logging.error("%s : Looks like an error updating the modular input parameter %s: %s" % (STANZA,rest_name,str(e),))   
...
def oauth2_token_updater(token):
    ...
    try:
        ...
        logging.error("%s : Looks like an error updating the oauth2 token: %s" % (STANZA,str(e)))
...
def handle_output(response,output,type,req_args,endpoint,oauth2): 
    ...
    try:
        ...
        logging.error("%s : Looks like an error handle the response output: %s" % (STANZA,str(e)))
