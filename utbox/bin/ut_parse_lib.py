...
def _loadIANAList(filename="suffix_list_iana.dat"):
	TLDFILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), filename) #"suffix_list_iana.dat")
...
def _loadMozillaList():
	...
	*.ck
	!www.ck
	"""
	TLDFILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), "suffix_list_mozilla.dat")
...
def loadTLDFile(tldlist="iana"):
	...
	if tldlist == "mozilla" :
		...
		return _loadIANAList("suffix_list_custom.dat") #dirty hack, but works ;)
	elif tldlist == "*" :
		# load all lists
		...
		TLD = {
		'regulars'  : list(set(Moz['regulars'] + Cus['regulars'] + Ian['regulars'])),
		'wildcards' : list(set(Moz['wildcards'] + Cus['wildcards'] + Ian['wildcards'])),
		'exceptions': list(set(Moz['exceptions'] + Cus['exceptions'] + Ian['exceptions']))
		}
