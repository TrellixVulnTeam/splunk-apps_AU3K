...
class RelayModAction(JsonModularInput):
    ...
    def get_password(self, realm, username, session_key):
        ...
        """
        namespace = urllib.quote(self.ACTION_NAMESPACE, safe='')
        key = urllib.quote("%s:%s:" % (realm, username), safe='')
        uri = '/servicesNS/nobody/%s/storage/passwords/%s' % (namespace, key)
        getargs = {'output_mode': 'json'}
        try:
            r, c = simpleRequest(uri, getargs=getargs, sessionKey=session_key, raiseAllErrors=True)
    ...
    def get_local_alert_actions(self, session_key):
        ...
        try:
            r, c = simpleRequest('alerts/alert_actions', getargs=getargs, sessionKey=session_key, raiseAllErrors=True)
        except Exception as e:
    ...
    @contextmanager
    def ensure_local_dispatch_dir(self, stanza, sid, mode=0755):
        """
    ...
    def run_action(self, cmd, settings, maxtime):
        ...
        try:
            timer.start()
            out, err = p.communicate(input=json.dumps(settings))
        finally:
            timer.cancel()
