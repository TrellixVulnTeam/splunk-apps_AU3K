...
def default_mapexp(x):
    ...
    return (
        x.startswith('tag::')
        or x in [
            '_time', '_raw', 'splunk_server', 'index',
...
class ModularAction(object):
    ...
    def __init__(self, settings, logger, action_name='unknown'):
        ...
        if self.results_file:
            ...
            if results_path.split(os.sep)[-1] == 'per_result_alert':
                self.info_file = os.path.join(
                    os.path.dirname(results_path), 'info.csv')
            else:
                self.info_file = os.path.join(
                    results_path, 'info.csv')
        self.job = {}
        ...
        for key, val in sorted(result.items()):
            ...
            if (vals and (real_key not in processed_keys)
                ...
                for v in vals:
                    ...
                    # check map
                    if mapexp(real_key):
                        _raw += ', %s="%s"' % ('orig_' + real_key.lstrip('_'), v)
                    else:
                        _raw += ', %s="%s"' % (real_key, v)
    ...
    def writeevents(self, index='summary', host=None, source=None, fext='common_action_model'):
        ...
        if self.events:
            ...
            for chunk in (
                    ...
                    for x in xrange(0, len(self.events), self.DEFAULT_CHUNK)):
    ...
    def queuework(self, exit_strategy=None, tries=3, raise_all=False):
        ...
        if isinstance(info_data, basestring):
            info = base64.b64encode(info_data)
        ...
        c = None
        for unused_x in xrange(0, tries):
            try:
                r, c = rest.simpleRequest(
                    self.CAM_QUEUE_URI,
                    sessionKey=self.session_key,
