...
class ModularActionQutils(object):
    ...
    def build_key(item):
        try:
            return '{0}@@{1}'.format(
                item['worker'],
                hashlib.sha1(item['action_name']+item['sid']).hexdigest()
            )
    ...
    def get_work(self, worker):
        ...
        uri = '{0}?query={1}'.format(
            ...
            urllib.quote_plus(json.dumps({'worker': worker}))
        )
    ...
    def unsave_work(self, query):
        ...
        uri = '{0}?query={1}'.format(
            self.CAM_QUEUE_URI,
            # quote_plus usage intentional here
            urllib.quote_plus(json.dumps(query))
        )
        ...
        sid_uri = '{0}?query={1}'.format(
            self.CAM_QUEUE_URI,
            # quote_plus usage intentional here
            urllib.quote_plus(json.dumps(sid_query))
        )
